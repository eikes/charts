...
class CountGraph < Graph
  ...
  def draw
    prepared_data.each_with_index do |row, row_count|
      row.each_with_index do |color, column_count|
        x = column_count * (item_width + inner_margin)
        y = row_count * (item_height + inner_margin)
        draw_item(x, y, color)
      end
    end
  end
  ...
  def width
    prepared_data.first.count * (item_width + inner_margin)
  end

  def height
    prepared_data.count * (item_height + inner_margin)
  end
